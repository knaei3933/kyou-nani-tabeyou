import { generateDeliveryLink } from '@/lib/delivery';
...
export const calculateDistanceFromUser = (
  ...
  return Math.round(R * c * 10) / 10; // 小数第1位まで
};
...
export const searchRestaurantsByFood = (foodKeywords: string[]): PilotRestaurant[] => {
  ...
  // 重複削除
  return results.filter((restaurant, index, self) => 
    index === self.findIndex(r => r.id === restaurant.id)
  );
};

// 配達アプリの深いリンク生成
export const generateDeliveryAppLink = (restaurant: PilotRestaurant, appName: string): string => {
  const baseUrls = {
    ubereats: 'https://www.ubereats.com/jp/store/',
    demaekan: 'https://demae-can.com/shop/detail/',
    wolt: 'https://wolt.com/ja/jpn/tokyo/restaurant/',
    menugo: 'https://menu.st/restaurant/',
    foodpanda: 'https://www.foodpanda.co.jp/restaurant/'
  };
  
  const deliveryId = restaurant.deliveryApps[appName as keyof typeof restaurant.deliveryApps];
  if (!deliveryId) return '';
  
  const baseUrl = baseUrls[appName as keyof typeof baseUrls];
  return baseUrl ? `${baseUrl}${deliveryId}` : '';
};

// レストラン統計情報
export const getRestaurantStats = () => {
  const totalCount = PILOT_RESTAURANTS.length;
  const areaStats = {
    渋谷: getRestaurantsByArea('渋谷').length,
    新宿: getRestaurantsByArea('新宿').length,
    恵比寿: getRestaurantsByArea('恵比寿').length
  };
  const popularCount = getPopularRestaurants().length;
  const deliveryAppStats = {
    ubereats: getRestaurantsByDeliveryApp('ubereats').length,
    demaekan: getRestaurantsByDeliveryApp('demaekan').length,
    wolt: getRestaurantsByDeliveryApp('wolt').length
  };
